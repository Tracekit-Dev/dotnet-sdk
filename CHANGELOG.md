...
### Added
- LLM auto-instrumentation for OpenAI and Anthropic APIs via `TracekitLlmHandler` (DelegatingHandler)
...
- Content capture option for request/response messages (`CaptureContent` config)
- Tool call detection and instrumentation for function calling
- PII scrubbing for captured content using regex and key-based patterns
- Provider auto-detection by HTTP request host
- Azure OpenAI support via `AzureOpenAiHandler`

### Fixed
- Registered `tracekit-llm` ActivitySource in tracer provider so LLM spans are actually exported
